...
class CryptoInfluencerApp:
    def __init__(self):
        ...
        # Allow users to select influencers
        ...
        # Load price data for each cryptocurrency
    ...
    def generate_chart(self, currency, price_data, influencer_data):
        ...
        # Define y-values for influencer annotations to avoid overlap
        y_values = [1 - i * 0.05 for i in range(len(filtered_influencers))]

        # Mark selected influencers' mentions on the chart
            ...
            if influencer in filtered_influencers:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertext=[f'{influencer}: {timestamp}'],  # Display influencer and timestamp on hover
                        hoverinfo='text'
                    )
                )

                # Add annotation for the influencer name
                fig.add_annotation(
                    x=timestamp,
                    y=price_data['close'].max() * y_values[i % len(y_values)],  # Adjust y value based on index
                    text=f'{influencer}',
                    showarrow=True,
                    arrowhead=2,
                    arrowsize=1,
        ...
        # Set the width of the chart dynamically based on the number of influencers
        width_factor = max(len(price_data['timestamp']) / 300, 1)  # Adjust the divisor for optimal width
        fig.update_layout(width=width_factor * 1000)  # You can adjust the multiplier for optimal width

        # Set the y-axis range to cover the entire data range
    ...
    def run(self):
        ...
        # Optional: Add other content to the second column
        # with st.container():
        #     st.write("This is the second column. You can add additional content here.")

# Create an instance of the CryptoInfluencerApp class and run the app
